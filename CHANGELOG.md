...
### Fixed
- **Every pattern no longer takes a fixed ~60s** ([#21](https://github.com/imagewize/wp-pattern-sentinel/issues/21)) — `page.waitForFunction(fn, arg, options)` takes its options third, but Sentinel passed `{ timeout }` second, where it was treated as the page-function argument and ignored. The "wait for save to start" check then always ran to the 60s page default, because `savePost()` had already finished by the time it began. The save step now takes ~0.5s instead of ~60s. On a 25-pattern block theme at `--concurrency=4`, the run time dropped from several minutes to ~16s.
- **Editor-load failures under concurrency are detected immediately and retried** ([#21](https://github.com/imagewize/wp-pattern-sentinel/issues/21)) — when all workers open `post-new.php` at once, ~150 script requests each can overflow a small PHP-FPM pool (Laravel Valet routes even static files through PHP), and dozens of editor scripts return 502. The editor then never boots, and Sentinel used to wait out the full timeout before reporting a `page_creation_error`. `createDraftPage` now fails as soon as any editor script returns a 5xx, and retries up to three times after a randomized backoff (3s, 8s, 15s, each plus up to 100% jitter), so workers don't collide again. The hard-coded 30s timeouts are gone as well; the page default (60s) remains as a backstop.
